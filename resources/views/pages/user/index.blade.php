            ...
            @forelse ($products as $item)
                <div class="col-lg-3 col-md-6">
                    <div class="single-product">
                        <img class="img-fluid" src="{{ asset('images/' . $item->image) }}" alt="Product Image">
                        <div class="product-details">
                            <h6>{{ $item->name }}</h6>
                            <div class="price">
                                @if($item->discount > 0)
                                    <h6>
                                        <del style="color: black;">{{ number_format($item->price, 0, ',', '.') }} Points</del>
                                    </h6>
                                    <h6 style="color: orange; font-weight: bold;">
                                        {{ number_format($item->price - ($item->price * $item->discount / 100), 0, ',', '.') }} Points
                                        <span style="color: orange; font-weight: normal;">{{ $item->discount }}%</span>
                                    </h6>
                                @else
                                    <h6 style="color: black;">{{ number_format($item->price, 0, ',', '.') }} Points</h6>
                                @endif
                            </div>
